...
- 핵심 데이터는 2026-09-05에 다시 확인한 KR 노출 함선 886척 기준이다. 지역별 기술 원본에만 추가된 새러토가(META)는 아직 KR·ALtoy에 없어 앱 표시 대상에서 제외했다.
...
- 벨파스트는 2026-09-02부터 전용 개조 아이템 `메이드장의 견벽`을 획득할 수 있다. KR 공식 완료 공지의 획득 개시 안내와 9월 2일 무점검 업데이트의 개조 화면·도감 오류 수정을 근거로 `canRemodel` 정정을 유지한다.
...
현재 이벤트 설정과 기존 함선 정정의 출처 우선순위는 `scripts/data/kr-active-events.json`에 기록한다. KR 공식 함선별 X 공지는 등장 및 기본 입수 분류, [8월 27일 업데이트 완료 안내](https://cafe.naver.com/azurlanekorea/574051)는 이벤트 명칭·기간·수령 기한·건조 확률의 1차 근거다. DCInside 전문 보존본은 네이버 카페 접근 불가 시에만 쓰는 예비 출처로 남긴다. BWiki는 건조 시간과 PT 교환량·이벤트 해역 드롭·누적 PT 세부만 교차검증한다. 벨파스트 개장은 완료 공지의 `메이드장의 견벽` 획득 개시와 [9월 2일 무점검 업데이트 안내](https://cafe.naver.com/azurlanekorea/574131)를 함께 근거로 사용한다. 보조 출처가 공식 KR 공지와 충돌하면 공식 KR 정보를 우선한다.
