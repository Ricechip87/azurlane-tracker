...
const KR_TECH_LUA = path.join(REFERENCE_ROOT, 'AzurLaneLuaScripts/KR/sharecfg/fleet_tech_ship_template.lua')
...
const cnTech = fs.existsSync(CN_TECH_LUA)
  ...
  : {}
const krTech = fs.existsSync(KR_TECH_LUA)
  ? parseFleetTechLua(fs.readFileSync(KR_TECH_LUA, 'utf8'))
  : {}
...
const counts = { 'cn-lua': 0, 'kr-lua': 0, 'tech-sheet': 0, existing: 0 }

const updated = characters.map(character => {
  const sheet = selectFleetTechSheetRecord(sheetRecords, character)
...
writeJsonAtomic(CHARS_PATH, updated)
console.log(`기술 데이터 반영 완료: CN Lua ${counts['cn-lua']}척, KR Lua ${counts['kr-lua']}척, 기술 시트 ${counts['tech-sheet']}척, 기존값 유지 ${counts.existing}척`)
